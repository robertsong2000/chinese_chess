...
// #57 Tempo Bonus(走子方加分):经典评估精化。中国象棋中先手是公认优势
// (与国际象棋 ~20-40cp 同量级)。在静态评估中给"轮到走子"的一方加 TEMPO_BONUS,
// 反映主动权(走子方能选择加强自身或威胁对方)。
// 取值保守(15):远低于兵 100,避免与子力评估冲突,同时足以让搜索在
// "均势但轮到走子更优"的节点正确排序。
// 启用方式:evaluateBoard(board, aiSide, side) 第三参数传入时启用;不传则保持对称
// (向后兼容,保护初始对称测试)。
